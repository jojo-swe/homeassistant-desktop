...
// --- CI workflow: Linux targets must match package.json ---
...
if (!linuxArgsMatch) {
  fail('build.yml: could not locate --linux target list');
} else {
  ...
  for (const target of linuxTargets) {
    if (!ciLinuxTargets.includes(target)) {
      fail(`build.yml is missing Linux target "${target}" (present in package.json)`);
    }
  }
  for (const target of ciLinuxTargets) {
    if (!linuxTargets.includes(target)) {
      fail(`build.yml has Linux target "${target}" not in package.json`);
    }
  }
}
